...
class YinshState:
    ...
    def clone(self):
        new_board =  self.game.get_initial_board()
        new_board[:] = self.board
        new_state = YinshState(self.game, new_board)
        new_state.phase = self.phase
        new_state.rings = copy.deepcopy(self.rings)
    ...
    def player_return(self, player_id):
...
class YinshGame:
    ...
    def get_direction_and_distance_from_coords(start_coord, end_coord):
        if start_coord[0] == end_coord[0]:
            if start_coord[1] < end_coord[1]:
                return Direction.HorizontalForward, end_coord[1] - start_coord[1]
            else:
                return Direction.HorizontalBackward, start_coord[1] - end_coord[1]
        elif start_coord[1] == end_coord[1]:
            if start_coord[0] < end_coord[0]:
                return Direction.VerticalForward, end_coord[0] - start_coord[0]
            else:
                return Direction.VerticalBackward, start_coord[0] - end_coord[0]
        elif start_coord[0] - end_coord[0] == start_coord[1] - end_coord[1]:
            if start_coord[0] < end_coord[0]:
                return Direction.DiagonalForward, end_coord[0] - start_coord[0]
            else:
                return Direction.DiagonalBackward, start_coord[0] - end_coord[0]
        else:
            return None, None
...
class YinshFrontend():
    def __init__(self):
        self.phase_strings = {}
        self.phase_strings[Phase.PlaceRing] = 'place_ring'
        self.phase_strings[Phase.MoveRing] = 'move_ring'
        self.phase_strings[Phase.RemoveRing] = 'remove_ring'
        self.phase_strings[Phase.RemoveChain] = 'remove_chain'
        self.phase_strings[Phase.GameOver] = 'game_over'

    def to_frontend_state(self, state):
        print('state board:', state.board)
        frontend_state = {}
        board_dict = {}
        for coord in state.game.coords:
            coord_key = f'{coord[0]},{coord[1]}'
            board_dict[coord_key] = int(state.board[coord])
        frontend_state['board'] = board_dict
        frontend_state['rings'] = state.rings
        frontend_state['turn_player'] = state.turn_player
        frontend_state['current_player'] = state.current_player
        frontend_state['phase'] = self.phase_strings[state.phase]
        frontend_state['is_game_over'] = True if state.phase == Phase.GameOver else False
        if state.phase == Phase.GameOver:
            frontend_state['winner'] = state.winner
        else:
            frontend_state['winner'] = None
        legal_actions = state.legal_actions()
        frontend_state['legal_actions'] = [YinshFrontend.to_frontend_action(state, action) for action in legal_actions]
        return frontend_state

    def from_frontend_state(self, state):
        pass

    def to_frontend_action(self, action):
        if action.type == ActionType.PlaceRing:
            return {'type': 'place_ring', 'coord': action.coord}
        elif action.type == ActionType.MoveRing:
            end_coord = YinshGame.CoordPlusVector(action.coord, action.direction, action.distance)
            return {'type': 'move_ring', 'start_coord': action.coord, 'end_coord': end_coord}
        elif action.type == ActionType.RemoveChain:
            end_coord = YinshGame.CoordPlusVector(action.coord, action.direction, 5)
            return {'type': 'remove_chain', 'start_coord': action.coord, 'end_coord': end_coord}
        elif action.type == ActionType.RemoveRing:
            return {'type': 'remove_ring', 'coord': action.coord}
        else:
    ...
    def from_frontend_action(self, frontend_action):
        ...
        if type == 'place_ring':
            coord = tuple(frontend_action['coord'])
            return YinshAction(ActionType.PlaceRing, coord=coord)
        elif type == 'move_ring':
            start_coord = tuple(frontend_action['start_coord'])
            end_coord = tuple(frontend_action['end_coord'])
            direction, distance = YinshGame.get_direction_and_distance_from_coords(start_coord, end_coord)
            return YinshAction(ActionType.MoveRing, coord=start_coord, direction=direction, distance=distance)
        elif type == 'remove_chain':
            start_coord = tuple(frontend_action['start_coord'])
            end_coord = tuple(frontend_action['end_coord'])
            direction, _ = YinshGame.get_direction_and_distance_from_coords(start_coord, end_coord)
            return YinshAction(ActionType.RemoveChain, coord=start_coord, direction=direction)
        elif type == 'remove_ring':
            coord = tuple(frontend_action['coord'])
            return YinshAction(ActionType.RemoveRing, coord=coord)
        else:
            raise Exception(f'Invalid action type: {type}')
